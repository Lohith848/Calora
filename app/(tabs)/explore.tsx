...
export default function ExploreScreen() {
  ...
  const stats = useMemo(() => {
    ...
    // Days where calories logged is > 0 but less than calorie goal
  ...
  const chartWidth = SW - 64 // padding clearance
  const chartMaxVal = useMemo(() => {
    const maxVal = stats.highest
    // Fallback if no logs exist, or scale so target goal fits
  ...
  // Draw target goal dashed line height
  ...
  return (
    ...
    >
          ...
          {weeklyDays.length === 0 ? (
            ...
            <Svg width={chartWidth} height={chartHeight}>
              ...
              {weeklyDays.map((day, i) => {
                ...
                // Scale bar height relative to maximum value
      ...
      <Card style={[s.insightAlert, { borderColor: ACCENT_DIM }]}>
...
const s = StyleSheet.create({
  ...
  chartCardVal: { fontSize: 16, fontWeight: '800', color: '#fff', marginTop: 2 },
  legendItem: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  legendLine: { width: 14, height: 2, backgroundColor: ACCENT, borderRadius: 1 },
